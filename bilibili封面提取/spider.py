...
class cover_extraction:
    ...
    def __init__(self, BV_AV):
        config = open(
            os.path.join(self.root_path, "config.yaml"), mode="r", encoding="utf-8"
        )
        cfg = config.read()
    ...
    def get_title(self, visible):
        ...
        title = (
            parsel.Selector(title_response)
            .xpath('//*[@id="viewbox_report"]/h1/span/text()')
            .get()
        )
        if visible:
    ...
    def download(self, url, title, bv_av):
        ...
        print(
            48,
            os.path.join(
                self.download_path, self.__word_list(title) + "___" + bv_av + ".jpg"
            ),
        )
    ...
    def __word_list(self, title):
        word_dict = {}
        with open(
            os.path.join(self.root_path, "word_list.txt"), mode="r", encoding="utf-8"
        ) as r_f:
            for i in r_f:
                line = i.split("\n")[0]
    ...
    def batch_download(self, before_page):
        ...
        page_url_list = [
            self.batch_url.format(page=str(page), keyword=self.key_word)
            for page in range(1, before_page + 1)
        ]
        for i in page_url_list:
            json_dict = json.loads(requests.get(i).text)
            data_list = json_dict["data"]["result"][-1]["data"]
            for j in data_list:
                pic_dict[self.__re_sub(j["title"]) + "-" + j["bvid"]] = (
                    "https:" + j["pic"]
                )
        pool = ThreadPool(self.thead_num)
        for key, value in pic_dict.items():
            pool.run(
                func=self.download, args=(value, key.split("-")[0], key.split("-")[1])
            )
        pool.close()
